...
제약이 많이 따르기 떄문에 조회(SELECT) 용도로 많이 사용

VIEW 작성법

CREATE [OR RELACE] FORCE | NOFORCE] VIEW 뷰이름 [컬럼 별칭]
AS 서브쿼리 (SELECT 문)
[WITH CHECK OPTION]
[WITH READ OLNY];

1) OR REPLACE
    기본 동일한 이름의 VIEW 가 존재하면 이름변경
    없으면 새로 생성
2)  FORCE | NOFORCE
...
--KH_WORKBOOK 뷰를 생성할 권한이 부족하는 것을 의미
이 문제를 해결하려면 데이터베이스 관리자 (DBA)나 시스템 관리자 한테 권한을 요청
...
CREATE VIEW 뷰이름 AS 1500 이상조 조회한 SELECT 문 작성

SELECT*FROM SNACKS WHERE PRICE >=1500;

--서울시 강남구에 위치 회사를 조회하는 뷰 COMPANY_IN GANGNAM
