...
--EMPLOYEE 테이블에서 몯느 사원의 이름, 급여 조회
--급여는 내림차순으로 정렬
SELECT EMP_ NAME, SALARY
FROM EMPLOYEE
...
ORDER BY DEPT_CODE; --ASC 기본 값이기 떄문에 오름차순 생략가능
...
--EMP_NAME : 현재 2번 쨰 자리에 위치하는 순서를 이용해서 ORDER BY를 구할수 있음
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SALARY BETWEEN 3000000 AND 6000000
ORDER BY 2DESC; --2 = EMP_NAME
--만약에 EMP_ID로 ORDER BY에서 정렬하길 원한다면 1을 작성
--만약에 SALARY로 ORDER BY 에서 정렬하길 원한다면 3을 작성

--ORDER BY 절에 수식 적용 --
--CMPLOYEE 테이블에서 이름, 연봉을 연봉 내림차순 조회
SELECT EMP_NAME, SALARY* 12
FROM EMPLOYEE
ORDER BY SALARY * 12 DESC; --월급 * 12달 = 연봉 내림차순 조회

--**ORDER BY 로 정렬을 진행할 경우에는
-- SELECT 절에서 작성된 컬럼을 그대로 따라 작성한 경우가 많음**--

/*ORDER BY 절에서 별칭 사용하기*/
--> SELECT 절에서 해석 이후 ORDER BY 절이 해석되기 때문에
...
-- WHERE 절의 경우 조건이기 떄문에 별칭 사용이 불가능
-- ORDER BY 별과를 가지고 결과 정리를 하는 표현방법
-- WHERE 은 결과를 나타내기 위해 찾는 조건문이기 떄문 
-- 조건이 뭔가 진행도 안됐는데 별칭부터 뭍인 것이기 때문
...
-- EMPLOYEE 테이블에서 이름, 부서 코드, 급여를
-- 부서코드 오름차순, 급여 내림차순 조회
-- 정렬해서 부서코드 어떻게 돼있고, 급여 가장 높은 금액만 궁금한 것
-- 누가 높게 받아? 가 궁금한 것은 아님
SELECT EMP_NAME. DEPT_CODE, ASLARY
FROM EMPLOYY
...
SELECT EMP_NAME AS "이름", DAPT_CODE AS "부서코드" JOB_CODE AS"직급코드"
FROM EMPLOYY
...
그 다음에
동일한 부서크드 내에서 직급 코드가 내림차순 정렬
D1의 부서의 경우 내림차순이기 때문에 J7이 J6보다 먼저 정렬

그 다음에 동일한 부서코드와 직급코드 내에서 이름이 오름차순으로 정렬

D1 부서에서는 직급코드 J7이 J6 보다 먼저 작성되고,
그 다음 J7은 단독이기 떄문에 정렬할 수 있는게 없음
J6에서는 2명 이상의 존재하기 떄문에 J6 직급의 직원들의 이름이 오름차순 정렬
...
FROM EMPLOYEE
ORDER BY 부서코드 ASC, 직급코드 DECS, 이름 ASC;
