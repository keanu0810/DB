...
PK가 지정된 컬럼에 삽임 될 값을 생ㅅ어할 떄 SEQUENCE 를 이용하면 좋음
EX) 1번쨰 회원 , 2번쨰 회원, 3번쨰 회원,.... N번쨰 회원

[작성법]
CREATE SEQUENCE 시퀀스이름
[START WITH 숫자] -- 처음 발생시킬 시작값 지정, 생략하면 자동으로 1이 기본값으로 설정
[INCREAMENT BY 숫자] -- 다음 값에 대한 증가값을 설정, 생략하면 자동으로 1씩 증가로 설정
[MAXVALUE 숫자 | NIMAXVALUE] -- 발생시킬 최대값 지정 ex) 최대 1000명만 기록하겠다.
[MINVALUE 숫자 | NOMINVALUE] -- 발생시킬 최소값 지정 ex) 21번부터 다시 시작하겠다.
[CYCLLE | NOCYCLE] -- 값 순환 여부 지정
[CACHE 바이트 크기 | NOCACHE -- 캐쉬메모리 기본값은 20BYTE 최소값 2BTTE
        -->시퀀스의 캐시 메모리는 할당된 크기만큼 미리 다음 값을 생ㅅ어해 저장해줌
        --> 시퀀스 호출 시 미리 저장되어진 값들을 가져와 반환하므로 매번 시퀀스를 생성해서 반환하는 것보다 DB 속도 향상
...
CREATE TABLE TB_TEST(
    TEST_NO NUMBER PRIMARY KEY, --ORACLE 지정한 최대 값 생성 38자리
    TEST_NAME VARCHAR2(30)NOT NULL
    );
    
CREATE SEQUENCE SEQ_TEST_NO
START WITH 100  -- 시작번호 100
INCRENENT BY    -- NEXTVAK  호출마다 5씩 증가
MAXVALUE        -- 증가 가능한 최대 값 150
NOMINVALUE      -- 최소값 없음
NOCYCLE         -- 반복없음
NOCACHE         -- 미리 생성해두는 시퀀스 번호 없음


-- 위 작성도니 내용 중 START 부터 NOCACHE 까지 모두 생략 가능
