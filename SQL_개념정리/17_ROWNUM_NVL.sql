...
--1. SALARY 월급이 높은 순서대로 출력한 다음 제대로 월급이 가장 높은 3명이 나온 것이 맞는지 확인
...
--이름, 급여 성관급(급여*보너스) 조회
--단, 성과급이 없으면 0으로 표시
SELECT EMP_ID, EMP_NAME, NVL(SALARY*BONUS, 0)AS"성과금"
FROM EMPLOYEE;
...
SELECT EMP_ID, EMP_NAME, NVL2(PHONE, '010' ||SUBSTR(PHONE, 010********),'없음') AS "전화번호"
FROM EMPLOYEE;
