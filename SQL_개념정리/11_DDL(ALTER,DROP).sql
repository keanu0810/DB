...
--ALTER(바꾸다, 수정하다)
--수정가능한 첫 컬럼 (추가/수정/삭제) 제약조건(추가/삭제) 이름변경(테이블,컬럼,제약조건)
...
--DEOT_COPY 테이블에 PK PRIMRY KEY 추가
...
--컬럼 추가 : ALTER TABLE 테이블명 ADD(컬럼명 데티어타입 [DEFAULT] '값'];

-- 컬럼 수정 : ALTER TABLE 테이블명 MODIFY 컬럼명 데이터 타입;(데이터 타입 변경)
-- 컬럼 수정 : ALTER TABLE 테이블명 MODIFY DEFAULT 값;(기본값 변경)

---> 데이터 타입 수정할 경우 컬럼에 저장된 크기 미만으로는 변경할 수 없음
------->       VARCHAR2(100) VARCHAR2(10) 데이터 타입이 10 초과되는 건 불가

-- 컬럼 삭제 : ALTER TABLE 테이블명 DROP (삭제할 컬럼명)
-->         ALTER TABLE 테이블명 DROP COLUMN 삭제할컬럼명;

--> 테이블에는 최소 1개 이상의 컬럼이 존재해야기 떄문에 모든 컬럼 삭제 X
...
--DEPT_COPY 테이블에 LNAME VARCGAR2(30) 기본값은 한국으로 추가
...
--ALTER TABLE 테이블 MODIFY 컬럼명 데이터타입;

ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR2(3)
-->주의할 점 기본에 저장 된 컬럼값이 3보다 크면 변경 불가
----> 컬럼값을 조절할 떄 (큰 값--> 작은값) 이미 저장된 데이터값이 변경할 값보다 크기 않은지 확인하고 조절
...
만약에 존재하지 않은 테이블을 삭제하려 하면 나오는 에러
*/
...
--3. 테이블 삭제
--[작성법]
--DROP TABLE 테이블명 [CASSCADE CONSTRAINTS];
...
--해경방법 2 : CASCADE CONSTRAINTS 옵션 사용
--> 제약조건까지 모두 삭제 조건을 없애 버려서 부모 자식 끊음
DROP TABLE TB1 CASCADE CONSTRAIOTS;
DROP TABLE TB2;
SELECT * FROM TB2;

--4. 제약조건 컬럼 테이블 이름변경 (RENAME)

--테이블 복사
--ALTER TABLE DEPT_COPY RENAME TO 새로운 테이블명;
CREATE TABLE DEPT_COPY AS SELECT * FROM department;
