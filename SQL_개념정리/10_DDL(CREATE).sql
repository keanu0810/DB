...
-- UNIQUE 는 컬럼에서 NULL이  한 개는 허용됨 두 개이상 안됨
...
-- parent key not found 부모키 인 USER_GRADE 에 40이라는 값이 없어서 넣을 수 없음
...
-- parent key not found 부모키 인 USER_GRADE 에 50이라는 값이 없어서 넣을 수 없음

INSERT INTO USER_GRADE VALUES (00, '임시회원'); -->정수에서 맨 앞에 0 사라짐
--> 핸드폰번호를 기입할 때는 VARCHAR2 를 사용해서 010모두 들어갈 수 있도록 설정
SELECT * FROM USER_GRADE;
...
--5.check 제약조건 : 컬럼에 기록되는 값에 조건을 설정
-- CGECK (컬럼명 비교연산자 비교값)
--비교값은 변하는 ㄱ밧이나 함수 사용 불가

CREATE TAVLE USER_CHECK )
USER_NO NUMVER PRIMARY KEY.
USER_ID VARCHAR2(20) UNIQUE.
USER_PW VARCHAR2(30) NOT NULL.
USER_NAME VARCHAR2(30).
GENDER VARCHAR2(10) CHECK(GENDER IN ('남','여'))
);
-- 앞으로 GENDER 컬럼에는 남이나 여라는 글자만 들어갈수 있음

INSERT INTO user_check VALUES( 1. 'user01', 'pw01','박철수','남자')'

INSERT INTO user_check VALUES( 1. 'user01', 'pw01','박철수','du')'
...
--2번 방법 따로 index에 기록한 다음 조건 설정만 할 경우
컬럼명 자료형 CONSTEAINT 인덱스에 기록할 이름 CHECK(컬럼명 IN ('조건1','조건2'))

--3번 방법 따로 index에 기록한 다음 조건 설정 한 경우 (어려줄 작성
컬럼명 자료형
