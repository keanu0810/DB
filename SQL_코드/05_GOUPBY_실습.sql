...
SELECT DEOT_CODE, ENT 
GORUP BY DEPT_CODEM NET_YE;

--각 부서별, 직급별, 입사일에 따른 사원수 조회
SELECT DEPT_CODE, GOB_CODE, HIRE_DATE, COUNT(*) AS"사원수"
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE, HIRE_DETE
ORDER BY DEPT_CODE, JOB_CODE, HIRE_DETE;
--공채 채용 후 기수별 남아 있는 사우너수 조회할 떄 사용
/*
ORA-00934: group function is not allowed here
00934. 00000 -  "group function is not allowed here"

->WHERE 절에서 집계 함수 (SUM, AVG, MAX, MIN, COUNT 등)
    사용할 수 없음
집계함수 (SUM,AVG,MAX, MIN, COUNT 등) HVING 이나 WHERE GROUP를
다른 방식으로 사용해서 작성
*/
...
SELECT DEPT_CODE, MIX(SALARY)AS "최소급여"
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY "최소급여" ASC;
...
--8. 급여가 300만 이상인 사원들의 부서명 사원수 조회
...
--9 부서별 보너스울 0.1 이상인 사원수 조회
-- COUNT AS "보너스받은사원수" 내림차순
SELECT DEPT_CODE, COUNT(*) AS "보너스받은사원수"
FROM EMPLOYEE
WHERE BONUS >=0.1
GROUP BY DEPT_CODE
ORDER BY "보너스받은 사원수" DESC;
--10 RMQDURK 400만원 이하인 사원들의 직급별 사원수 조회
-- CONUT AS "사원수" 내림차순
SELECT JOB_CODE, COUNT(*) AS "사원수"
FROM EMPLOYEE
...
--11 급여가 500만원 이상인 사우너들의 부서별 평균 급여 조회
--AVG AS "평균급여" SOFLACKTNS
...
ORDER BY DEPT_CODE;
/*
D1 D5 D6 부서는 70년 평균 급여가 300이상 인 부서
D1	3220000
D5	3500000
D6	3900000
*/
