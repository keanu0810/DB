...
--2. EMPOYEE 테이블에서 모든 직원의 급여를 내림차순 정렬 조회
-- 모두보기
SELECT * FROM EMPLOYEE
...
--별칭이용해서 직원이름 내림차순 정렬
SELECT EMP AS"사번", EMP_NAME AS"이름" FROM EMPLOYEE
ORDER BY "이름" ASC;
...
--6. EMPLOYEE 테이블에서 모든 직원의 이메일을 "이메일"VYRL
-- EMAIL 오름차순으로 정렬
-- 전화번호 AS"전화번호"표기
-- EMAIL 내에서는 전화번호를 내림차순으로 정렬
SELECT EMP_ID, EMP_NAME, EAMIL AS "이메일",PHONE AS "전화번호"
FROM EMPLOYEE
...
SELECT EMP_NAME AS "직원명", HIRE_DETE AS "입사일자"
FROM EMPLOYEE
